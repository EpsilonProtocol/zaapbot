...
export const sendEthHandler = async (
  ...
  try {
    await twitterClient.sendDm(
      recieverId,
      `🎉 User @${sender.twitter.username} has send you funds using a Peanut link (read more+link)! ✨ Follow this link (${link}) to claim ${amount} ETH`,
    );
  } catch (err) {
    console.log(`Cannot send Dm to ${reciever}`, err);
