...
contract SafeDelegatedProxy {
    ...
    function getMaxContractAllowance(
        address owner,
        address target
    ) public view returns (uint256) {
    ...
    function setMaxAmountToPayForNFT(
        ...
        bytes32 key = generateBuyAllowanceKey(owner, nft, tokenId);
        allowances721[key] = Purchase721Info({
            initiated: false,
            maxPrice: amount,
    ...
    function setMaxContractAllowance(
        address owner,
        address target,
        uint256 amount,
        address spender
        ...
        require(msg.sender == owner, "Only owner can set allowance");
        bytes32 key = generateBuyAllowanceKey(owner, target, 0);
        allowancesNative[key] = PurchaseNativeInfo({
            initiated: false,
            maxSpend: amount,
            gnosisSafeInstance: Executor(spender)
        });
    ...
    function buyNFT(
        ...
        bytes32 key = generateBuyAllowanceKey(owner, nft, tokenId);
        require(
            amount <= allowances721[key].maxPrice,
    ...
    function createPeanutLink(
        ...
        bytes32 key = generateBuyAllowanceKey(
            owner,
            peanutForwarderContract,
            0
        );
        require(
            amount <= allowancesNative[key].maxSpend,
            "Price less than expected"
        );
        // Protect against re-entrancy
        ...
        bytes memory data = abi.encodePacked(amount, pubKey20); // claiming public key
    ...
    function canSellNFT(
        ...
        return (
            sellingallowances721[key].canBeSold,
            sellingallowances721[key].minPrice
        );
    }
    ...
    function sellNFT(
        ...
        require(
            msg.value >= sellingallowances721[key].minPrice,
    ...
    function transferNFT(
        ...
        require(
            sellingallowances721[key].canBeTransferred,
            "Not transferrable"
        );
